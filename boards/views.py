    ...
    def post(self, request):
        ...
        if form.is_valid() and user.is_authenticated:
            ...
            try:
                ...
                form.save_m2m()
                logger.info(
                    f"{user} [{get_client_ip(request)}] added a new "
                    f"board: {board.name}"
                )
            except IntegrityError:
                logger.warning(
                    f"{user} [{get_client_ip(request)}] tried to add a new "
...
class TopicListView(ListView):
    ...
    def get_queryset(self):
        ...
        logger.info(
            f"{self.request.user} [{get_client_ip(self.request)}] selected "
    ...
    def post(self, request, board_pk):
        ...
        if user.is_authenticated and form.is_valid():
            ...
            if new_board_name and user == board.owner:
                ...
                logger.info(
                    f"{user} [{get_client_ip(request)}] renamed "
                    f"board: {old_board_name} to {board.name}"
                )
            ...
            if user == board.owner:
                if set(board.contributor.all().all()) != set(contributors):
                    ...
                    logger.info(
                        f"{user} [{get_client_ip(request)}] changed contributors for "
                        f"board: {board.name}"
                    )
                board.save()
...
def new_topic(request, board_pk):
    ...
    if request.method == "POST":
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            form2.save_m2m()
            logger.info(
                f"{request.user} [{get_client_ip(request)}] created new "
...
class PostListView(ListView):
    ...
    def post(self, request, **kwargs):
        ...
        logger.info(
            f"{request.user} [{get_client_ip(request)}] deleted "
...
def add_post_to_topic(request, board_pk, topic_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                f"{request.user} [{get_client_ip(request)}] added a new "
...
class PostUpdateView(UpdateView):
    ...
    def delete_post(self):
        ...
        post_candidate_delete = get_object_or_404(Post, pk=deleted_post_pk)
        logger.info(
            f"{self.request.user} [{get_client_ip(self.request)}] deleted "
            f"post: {post_candidate_delete.post_subject}"
        )
        post_candidate_delete.delete()
        return True
    ...
    def form_valid(self, form):
        ...
        if self.delete_post():
            ...
            form.save_m2m()
            logger.info(
                f"{self.request.user} [{get_client_ip(self.request)}] updated "
                f"post {post.post_subject}"
            )
            topic_url = reverse(
                "topic_posts",
