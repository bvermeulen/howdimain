...
class IntraDayView(View):
    ...
    def get(self, request, source, symbol):
        ...
        try:
            ...
            prc_change = (
                100 * (float(latest_close) - float(initial_open)) / float(initial_open)
            )
        ...
        if initial_open and latest_close and prc_change:
            ...
            subcaption = "".join(
                [
                    Stock.objects.get(symbol_ric=symbol).currency.currency,
        ...
        caption = "".join(
            [Stock.objects.get(symbol_ric=symbol).company, " (", symbol, ")"]
        )
        ...
        logger.info(
            f"{user} [{get_client_ip(request)}] is looking "
...
class HistoryView(View):
    ...
    def get(self, request, source, symbol, period):
        ...
        if history_trades:
            start_period = history_trades[0].date - datetime.timedelta(
                days=int(period_num * 365)
            )
        ...
        time_series.AddAttribute(
            "extensions", {"customrangeselector": {"enabled": "1"}}
        )
        time_series.AddAttribute(
            "xaxis", {"initialinterval": {"from": start_date, "to": end_date}}
        )
        time_series.AddAttribute(
            "yaxis",
            [
