...
def newspage(request):
    ...
    update_news_true = (
        (ns.current_news_site != ns.news_site)
        or (ns.item == 0 and not (button_cntr or button_title))
        or (ns.updated != today)
        or (button_title == "refresh")
    )
    if update_news_true:
        feed_items = update_news(
    ...
    if ns.news_items == 0:
        default_site = str(
            UserNewsSite.objects.get(user__username="default_user").news_sites.first()
        )
        ns.error_message = (
            f"Newssite {ns.current_news_site} is not available, "
            f"revert to default site {default_site}"
        )
        logger.warning(
            f"{ns.current_news_site} is not available, revert to default site"
        )
    ...
    logger.info(f"{user} [{ip_address}] browsing news: {ns.current_news_site}")
